  ...
  const roleInstructions =
    sender.role === "customer"
      ? [
          ...
          "If a customer wants to update an already-submitted order that is still awaiting restaurant confirmation, use amend_submitted_order with their order reference and only the exact changes they requested. This can change an item quantity, remove an item with quantity 0, add an available menu item, switch pickup/delivery, or change the delivery address. If the restaurant has already accepted or started the order, explain that it can no longer be edited and offer cancellation when eligible.",
          "If a customer wants to cancel a submitted order, use cancel_order with their order reference or look up their latest order first. Active orders may be cancelled until completion; the owner is notified of customer cancellation.",
